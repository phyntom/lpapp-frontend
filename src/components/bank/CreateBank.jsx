...
import { MDBContainer, MDBCard, MDBRow, MDBCol, MDBCardTitle, MDBCardText } from 'mdbreact';
import Form, { GroupItem, SimpleItem, RequiredRule, ButtonItem, StringLengthRule } from 'devextreme-react/form';
...
class CreateBank extends Component {
    ...
    if (this.state.mode === 'insert') {
      BankService.createBank(bank)
        ...
        .catch((error) => {
          ...
          if (!error.response) {
            FeedBack.triggerMessage('error', `Error while fetcing banks | Reason ${error.message}`, 1000);
          } else {
            FeedBack.triggerMessage('error', `Bank not created successfully | Reason ${error.response.data.message}`, 3000);
            bankStore.reset();
          }
        });
    } else {
  ...
  render() {
    const { bankStore } = this.props;
    const { showColon, labelLocation, minColWidth, colCount, formTitle } = this.state;
    return (
      <MDBContainer>
        <MDBRow center gutters='2'>
          <MDBCol md='6'>
            <form onSubmit={this.onFormSubmit}>
              <Form id={'form'} formData={bankStore.bank} showColonAfterLabel={showColon} labelLocation={labelLocation} minColWidth={minColWidth} colCount={colCount}>
                <GroupItem caption={'Bank Details'}>
                  ...
                  <SimpleItem dataField={'discount'} caption={'Discount'} editorType={'dxTextBox'}>
